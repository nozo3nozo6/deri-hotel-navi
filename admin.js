...
// ダッシュボードタスクカード用 (multi-statement onclick の置換)
function dashGoToFlaggedReports(){switchTab('reports');document.getElementById('rep-flag-f').value='flagged';repPage=0;renderReports();}
function dashGoToPendingShops(){switchTab('shops');document.getElementById('shop-appr-f').value='registered';renderShops();}
function dashGoToHotelRequests(){switchTab('hotel-requests');}
function dashGoToCorrections(){switchTab('hotel-corrections');}
function dashGoToPlanRequests(){switchTab('plan-requests');}
// updateGoodPointCategory を data-onchange 経由で呼ぶラッパー
function updateGoodPointCategoryFromEl(el){const id=parseInt(el.dataset.gpId);if(typeof updateGoodPointCategory==='function')updateGoodPointCategory(id,el.value);}
// 契約期限変更を data-onchange 経由で
function setContractExpiryFromEl(el){const id=parseInt(el.dataset.cid);const shopId=el.dataset.shopId;if(typeof setContractExpiry==='function')setContractExpiry(id,shopId,el.value);}
// 広告エリア選択を data-onchange 経由で
function onAdPrefChangeFromEl(el){if(typeof onAdPrefChange==='function')onAdPrefChange(el.dataset.adPrefix,el.dataset.adLevel);}
function onAdMajorChangeFromEl(el){if(typeof onAdMajorChange==='function')onAdMajorChange(el.dataset.adPrefix,el.dataset.adLevel);}
function onAdDetailChangeFromEl(el){if(typeof onAdDetailChange==='function')onAdDetailChange(el.dataset.adPrefix,el.dataset.adLevel);}

// === drag-and-drop イベント委譲 ===
// data-dnd-id / data-dnd-type を持つ要素で:
//   ondragstart → dndStart(event, id, type)  / ondragover → dndOver(event) /
//   ondragleave → dndLeave(event) / ondragend → dndEnd(event) / ondrop → dndDrop(event, id)
document.addEventListener('dragstart', function(e) {
    const el = e.target.closest('[data-dnd-id]');
    if (!el || typeof dndStart !== 'function') return;
    dndStart(e, parseInt(el.dataset.dndId), el.dataset.dndType);
});
document.addEventListener('dragover', function(e) {
    const el = e.target.closest('[data-dnd-id]');
    if (!el || typeof dndOver !== 'function') return;
    dndOver(e);
});
document.addEventListener('dragleave', function(e) {
    const el = e.target.closest('[data-dnd-id]');
    if (!el || typeof dndLeave !== 'function') return;
    dndLeave(e);
});
document.addEventListener('dragend', function(e) {
    const el = e.target.closest('[data-dnd-id]');
    if (!el || typeof dndEnd !== 'function') return;
    dndEnd(e);
});
document.addEventListener('drop', function(e) {
    const el = e.target.closest('[data-dnd-id]');
    if (!el || typeof dndDrop !== 'function') return;
    dndDrop(e, parseInt(el.dataset.dndId));
});
...
// ===== ダッシュボード =====
async function loadDashboard(){const d=await apiGet('dashboard');document.getElementById("s-hotels").textContent=(d.hotel_count||0).toLocaleString();document.getElementById("s-reports").textContent=(d.report_count||0).toLocaleString();document.getElementById("s-shops").textContent=(d.shop_count||0).toLocaleString();document.getElementById("s-canrate").textContent=d.report_count?Math.round(d.can_count/d.report_count*100)+"%":"—";const flagCount=d.flag_count||0;const shopPendCount=d.shop_pend_count||0;const hreqPendCount=d.hreq_pend_count||0;const taskEl=document.getElementById("dash-tasks");const tasks=[];if(flagCount>0)tasks.push(`<div class="task-card tc-red" data-action="dashGoToFlaggedReports"><div class="tc-icon">&#x1F6A9;</div><div class="tc-body"><div class="tc-count">${flagCount}</div><div class="tc-label">報告未対応の投稿</div></div></div>`);if(shopPendCount>0)tasks.push(`<div class="task-card tc-orange" data-action="dashGoToPendingShops"><div class="tc-icon">&#x1F3EA;</div><div class="tc-body"><div class="tc-count">${shopPendCount}</div><div class="tc-label">審査待ちの店舗</div></div></div>`);if(hreqPendCount>0)tasks.push(`<div class="task-card tc-blue" data-action="dashGoToHotelRequests"><div class="tc-icon">&#x1F3E8;</div><div class="tc-body"><div class="tc-count">${hreqPendCount}</div><div class="tc-label">未確認の掲載リクエスト</div></div></div>`);const corrPendCount=d.corr_pend_count||0;if(corrPendCount>0)tasks.push(`<div class="task-card tc-orange" data-action="dashGoToCorrections"><div class="tc-icon">&#x26A0;&#xFE0F;</div><div class="tc-body"><div class="tc-count">${corrPendCount}</div><div class="tc-label">未対応の情報修正</div></div></div>`);const planReqPendCount=d.plan_req_pend_count||0;if(planReqPendCount>0)tasks.push(`<div class="task-card tc-blue" data-action="dashGoToPlanRequests"><div class="tc-icon">&#x1F4CB;</div><div class="tc-body"><div class="tc-count">${planReqPendCount}</div><div class="tc-label">未処理のプラン申込</div></div></div>`);if(tasks.length){taskEl.innerHTML=tasks.join("");taskEl.style.display="grid";}else{taskEl.style.display="none";}
// ホテル投稿
...
async function loadShops(){
    const data=await apiGet('list','table=shops&order=created_at&dir=desc');
    shopsData=Array.isArray(data)?data:[];
    try{
        // 🎁マーク判定: キャンペーン契約があり、かつ通常の有料契約(plan_id>1, is_campaign=0)が無い店舗のみ
        // 通常プランも併用している店舗（キャンペーンは付録扱い）には🎁を付けない
        const sc=await apiGet('list','table=shop_contracts');
        const allContracts=Array.isArray(sc)?sc:[];
        const byShop={};
        allContracts.forEach(c=>{
            const sid=String(c.shop_id);
            if(!byShop[sid])byShop[sid]={hasCampaign:false,hasPaidNormal:false};
            if(c.is_campaign)byShop[sid].hasCampaign=true;
            else if(Number(c.plan_id)>1)byShop[sid].hasPaidNormal=true;
        });
        _campaignShopIds=new Set(
            Object.entries(byShop)
                .filter(([_,v])=>v.hasCampaign&&!v.hasPaidNormal)
                .map(([id,_])=>id)
        );
    }catch(e){}
    updateShopBadge();
...
async function loadShopContracts(shopId,plans){
  ...
  plans.forEach(p=>{
    ...
    if(MULTI_PLAN_IDS.has(p.id)){
      ...
      planHtml+=`<button data-action="removeOneContract" data-arg1="${shopId}" data-arg2="${p.id}" style="width:28px;height:28px;border:1px solid #ccc;border-radius:4px;background:#fff;cursor:pointer;font-size:16px;line-height:1;${cnt===0?'opacity:0.3;pointer-events:none':''}">−</button>`;
      planHtml+=`<span style="font-size:14px;font-weight:700;min-width:20px;text-align:center;">${cnt}</span>`;
      planHtml+=`<button data-action="addOneContract" data-arg1="${shopId}" data-arg2="${p.id}" style="width:28px;height:28px;border:1px solid #ccc;border-radius:4px;background:#fff;cursor:pointer;font-size:16px;line-height:1;">＋</button>`;
      planHtml+=`</div></div>`;
    }else{
      ...
      const icon=isActive?'✅ ':'';
      planHtml+=`<button data-action="toggleShopContract" data-arg1="${shopId}" data-arg2="${p.id}" data-arg3="${isActive}" style="${style}">${icon}${esc(p.name)}（¥${p.price.toLocaleString()}）</button>`;
    }
  });
  planHtml+='</div>';
...
function renderGoodPoints(){document.getElementById("goodpoints-sub").textContent=`全 ${goodPointsData.length} 件`;document.getElementById("goodpoints-list").innerHTML=goodPointsData.length===0?`<div style="text-align:center;padding:20px;color:var(--ink-3);font-size:13px;">まだ登録されていません</div>`:goodPointsData.map(c=>{const lbl=c.label||'(ラベル未設定)';return`<div class="cond-row" draggable="true" data-dnd-id="${c.id}" data-dnd-type="loveho_good_points" style="${c.is_active?'':'opacity:0.4;'}"><span class="drag-handle">&#9776;</span><span class="cond-order">${c.sort_order}</span><span class="cond-label">${esc(lbl)}</span><select data-onchange="updateGoodPointCategoryFromEl" data-gp-id="${c.id}" style="padding:4px 6px;border:1px solid var(--border);border-radius:4px;font-size:12px;margin-right:6px;"><option value="設備・お部屋"${c.category==='設備・お部屋'?' selected':''}>設備・お部屋</option><option value="サービス・利便性"${c.category==='サービス・利便性'?' selected':''}>サービス・利便性</option></select><button class="btn b-btn-gold" data-action="toggleGoodPoint" data-arg1="${c.id}" data-arg2="${c.is_active}">${c.is_active?'無効にする':'有効にする'}</button><button class="btn b-btn-gold" data-action="editGoodPoint" data-arg1="${c.id}" data-arg2="${esc(lbl)}">編集</button><button class="btn b-btn-red" data-action="deleteGoodPoint" data-arg1="${c.id}" data-arg2="${esc(lbl)}">削除</button></div>`;}).join("");}
...
// 内部的に code (loveho_reports.entry_method に保存する識別子) を持つが、UI では label のみ編集.
// 新規追加時の code は em_<8桁ランダム> で自動生成。既存4種 (front/direct/lobby/waiting) は seed の code を維持.
let entryMethodsData=[];
function genEntryMethodCode(){
    let code;
    do { code='em_'+Math.random().toString(36).substring(2,10); }
    while(entryMethodsData.some(x=>x.code===code));
    return code;
}
async function loadEntryMethods(){
    const data=await apiGet("list","table=loveho_entry_methods&order=sort_order&dir=asc");
    entryMethodsData=Array.isArray(data)?data:[]; // 防御: エラー時もlength取得可能に
    renderEntryMethods();
}
function renderEntryMethods(){
    document.getElementById("entrymethod-sub").textContent=`全 ${entryMethodsData.length} 件`;
    document.getElementById("entrymethod-list").innerHTML=entryMethodsData.length===0?`<div style="text-align:center;padding:20px;color:var(--ink-3);font-size:13px;">まだ登録されていません</div>`:entryMethodsData.map(c=>`<div class="cond-row" draggable="true" data-dnd-id="${c.id}" data-dnd-type="loveho_entry_methods"><span class="drag-handle">&#9776;</span><span class="cond-order">${c.sort_order}</span><span class="cond-label">${esc(c.label)}</span><button class="btn b-btn-gold" data-action="editEntryMethod" data-arg1="${c.id}" data-arg2="${esc(c.label)}">編集</button><button class="btn b-btn-red" data-action="deleteEntryMethod" data-arg1="${c.id}" data-arg2="${esc(c.label)}">削除</button></div>`).join("");
}
async function addEntryMethod(){
    const input=document.getElementById("new-entrymethod");
    const label=input.value.trim();
    if(!label){toast("入室方法を入力してください");return;}
    const code=genEntryMethodCode();
...
async function editEntryMethod(id,label){
    const newLabel=prompt("入室方法を編集してください:",label);
...
async function deleteEntryMethod(id,label){
...
function renderHotelRequests(){
  ...
  document.getElementById("hreq-tbody").innerHTML=data.map(function(r){
    ...
    var toggleBtn=isConfirmed
      ?'<button class="btn b-btn-gray" data-action=\"toggleHreqStatus\" data-arg1=\"'+id+'\">未確認に戻す</button>'
      :'<button class="btn b-btn-green" data-action=\"toggleHreqStatus\" data-arg1=\"'+id+'\">確認済</button>';
    var actionBtns='<div class="btn-row">'+toggleBtn
      +'<button class="btn b-btn-gold" data-action=\"openHreqEdit\" data-arg1=\"'+id+'\">編集→登録</button>'
      +'<button class="btn b-btn-red" data-action=\"deleteHotelRequest\" data-arg1=\"'+id+'\">削除</button>'
      +'</div>';
    return '<tr>'
...
function renderCorrections(){
  ...
  document.getElementById("corr-tbody").innerHTML=data.length===0?`<tr class="erow"><td colspan="6">情報修正リクエストはありません</td></tr>`:data.map(function(r){
    ...
    let actionBtns='';
    if(r.status==="pending"){
      actionBtns='<button class="btn b-btn-gray" style="font-size:10px;background:#28a745;color:#fff;" data-action=\"resolveCorrection\" data-arg1=\"'+r.id+'\">✅ 対応済み</button> <button class="btn b-btn-gray" style="font-size:10px;background:#dc3545;color:#fff;" data-action=\"rejectCorrection\" data-arg1=\"'+r.id+'\">❌ 却下</button>';
    }else{
      actionBtns='<button class="btn b-btn-gray" style="font-size:10px;" data-action=\"reopenCorrection\" data-arg1=\"'+r.id+'\">🔄 未対応に戻す</button> <button class="btn b-btn-gray" style="font-size:10px;background:#dc3545;color:#fff;" data-action=\"deleteCorrection\" data-arg1=\"'+r.id+'\">🗑 削除</button>';
    }
    return`<tr>
...
async function renderTargetDropdowns(container,prefix,level){
    ...
    let html='<select id="'+prefix+'-target-pref" class="f-input" data-onchange="onAdPrefChangeFromEl" data-ad-prefix="'+prefix+'" data-ad-level="'+level+'"><option value="">都道府県を選択</option>'+prefs.map(p=>'<option>'+esc(p)+'</option>').join('')+'</select>';
    if(level==='area'||level==='town'||level==='spot'){
        html+='<select id="'+prefix+'-target-major" class="f-input" style="margin-top:6px;display:none;" data-onchange="onAdMajorChangeFromEl" data-ad-prefix="'+prefix+'" data-ad-level="'+level+'"><option value="">エリアを選択</option></select>';
    }
    if(level==='town'||level==='spot'){
        html+='<select id="'+prefix+'-target-detail" class="f-input" style="margin-top:6px;display:none;" data-onchange="onAdDetailChangeFromEl" data-ad-prefix="'+prefix+'" data-ad-level="'+level+'"><option value="">詳細エリアを選択</option></select>';
    }
    if(level==='spot'){
    ...
    document.getElementById('ad-contracts-body').innerHTML=rows.map(r=>{
        ...
        return`<tr>
            ...
            <td style="white-space:nowrap;">
                ${r.status==='active'?`<button class="btn b-btn-gold" style="font-size:11px;padding:4px 8px;" data-action="toggleAdContract" data-arg1="${r.id}" data-arg2="paused">停止</button>`:''}
                ${r.status==='paused'?`<button class="btn b-btn-green" style="font-size:11px;padding:4px 8px;" data-action="toggleAdContract" data-arg1="${r.id}" data-arg2="active">再開</button>`:''}
                <button class="btn b-btn-red" style="font-size:11px;padding:4px 8px;" data-action="deleteAdContract" data-arg1="${r.id}" data-arg2="${r.shop_id}">削除</button>
            </td>
        </tr>`;
    }).join('');
}
