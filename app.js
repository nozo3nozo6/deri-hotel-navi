...
let SHOP_DATA = null; // shop info for banner
...
async function initShopMode() {
    ...
    const curMode = new URLSearchParams(window.location.search).get('mode') || 'men';
    ...
    if (titleBar) {
        ...
        titleBar.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:wrap;">
            <span style="font-size:14px;font-weight:700;">${modeIcon} ${shop.shop_name} の対応情報</span>
            <a href="portal.html?mode=${curMode}" style="font-size:11px;color:var(--text-3);text-decoration:none;margin-left:8px;">全ての情報を見る →</a>
        </div>`;
    }
}
