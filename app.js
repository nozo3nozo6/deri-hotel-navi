...
// CDNから読み込んだグローバル supabase をそのまま使用（再宣言エラー回避）
const supabase = window.supabase;

if (!supabase) {
    console.error('Supabaseが読み込まれていません。portal.htmlの<head>にCDNスクリプトがあるか確認してください。');
}
...
async function loadLevel(level = 'japan', parentCode = null) {
    ...
    if (btnBack) btnBack.style.display = level === 'japan' ? 'none' : 'block';
    ...
    const { data, error } = await query.not('name', 'is', null).order('name');
...
async function fetchHotels() {
    ...
    const { data: hotels, error } = await q.order('name').limit(100);

    if (error) {
        console.error(error);
        listContainer.innerHTML = `<p style="color:red;">エラーが発生しました</p>`;
        return;
    }
