...
function checkRateLimit($ip) {
    $lockFile = sys_get_temp_dir() . '/agent_auth_lock_' . md5($ip) . '.json';
...
function handleLogin() {
    ...
    $pdo = DB::conn();
    $stmt = $pdo->prepare('SELECT * FROM agent_users WHERE username = ? LIMIT 1');
    ...
    echo json_encode(['success' => true, 'agent' => ['id' => $user['id'], 'username' => $user['username'], 'name' => $user['name']]]);
}
...
function handleCheck() {
    if (empty($_SESSION['agent_id'])) { echo json_encode(['authenticated' => false]); return; }
    $lastActivity = $_SESSION['last_activity'] ?? 0;
    if (time() - $lastActivity > SESSION_TIMEOUT) { session_destroy(); echo json_encode(['authenticated' => false, 'reason' => 'timeout']); return; }
    $_SESSION['last_activity'] = time();
    echo json_encode(['authenticated' => true, 'agent' => ['id' => $_SESSION['agent_id'], 'username' => $_SESSION['agent_username'], 'name' => $_SESSION['agent_name']]]);
}

function handleChangePassword() {
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') { http_response_code(405); echo json_encode(['success' => false, 'message' => 'POSTのみ']); return; }
    if (empty($_SESSION['agent_id'])) { http_response_code(401); echo json_encode(['success' => false, 'message' => '認証が必要です']); return; }
    $input = json_decode(file_get_contents('php://input'), true);
    ...
    $stmt = $pdo->prepare('SELECT * FROM agent_users WHERE id = ? LIMIT 1');
    ...
    $stmt = $pdo->prepare('UPDATE agent_users SET password_hash = ? WHERE id = ?');
    $stmt->execute([$newHash, $agentId]);
