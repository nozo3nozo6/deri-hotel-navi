...
if ($isPureTopPage) {
    ...
    // --- モード別 独自トップコンテンツ（重複ページ化を防ぎ、各ジャンルトップを固有化）---
    // 課題: モードトップ5ページ(/deli/,/jofu/,/same-m/,/same-f/,/este/)はHTMLがほぼ同一で
    //       Googleに重複統合され、/deli/以外が「代替ページ」扱いでインデックスされない。
    // 対策: ジャンル固有の導入文・ホテル選びのヒント・FAQ(+FAQPage構造化データ)をSSRで注入し、
    //       各トップを独自コンテンツ化する。content未定義のモードは従来通り（安全に段階展開）。
    $MODE_TOP_CONTENT = [
        'men' => [
            'intro_h2' => 'デリヘルを呼べるホテルの選び方｜失敗しないために知っておきたいこと',
            'intro' => [
                'デリヘル（デリバリーヘルス）は、女性キャストがホテルや自宅に出張してサービスを提供する風俗です。利用場所として多いのがホテルですが、すべてのホテルでデリヘルを呼べるわけではありません。フロントで断られたり、出禁になってしまったりするケースもあり、「このホテルは本当に呼べるのか」を事前に知っておくことが、失敗しないための大きなポイントです。',
                'デリヘルを呼びやすいのは、フロントを通らずに部屋へ行けるカードキー式・直通エレベーターのホテルや、来訪者に寛容なビジネスホテル・シティホテルです。一方で、ラブホテルやリゾートホテル、一部のシティホテルでは対応が分かれます。',
                'YobuHoでは、実際にデリヘルを利用した人の「呼べた／呼べなかった」口コミと、届出確認済みのデリヘル店舗の案内実績から、ビジネスホテル・シティホテルを中心にデリヘルを呼べるホテルを地域や駅名から検索できます。',
            ],
            'tips_h3' => 'デリヘルを呼びやすいホテルの特徴',
            'tips' => [
                ['t' => '「呼べた」口コミ実績の多いホテル', 'd' => '実際に呼べた報告が集まっているホテルなら、フロント対応や入室のしやすさが事前に分かり、断られるリスクを避けられます。'],
                ['t' => 'カードキー式・直通エレベーター', 'd' => 'フロントを通らず部屋へ直行できる構造は、キャストの出入りが目立たず、デリヘルを呼びやすいホテルの代表格です。'],
                ['t' => '歓楽街・駅近で出張利用の多いビジネスホテル', 'd' => 'デリヘル利用に慣れたホテルが多く、キャストの移動や交通費の面でも有利なエリアです。'],
                ['t' => '来客に寛容で出禁リスクの低いホテル', 'd' => '来訪者の確認が緩やかなホテルは、トラブルや出禁になりにくい傾向があります。'],
            ],
            'faq' => [
                ['q' => 'デリヘルはビジネスホテルやシティホテルに呼べますか？', 'a' => '多くのビジネスホテル・シティホテルでデリヘルを呼ぶことは可能です。ただしホテルごとに対応が分かれ、フロントで断られるケースもあるため、YobuHoでは利用者の「呼べた／呼べなかった」口コミと店舗の案内実績から事前に確認できます。'],
                ['q' => 'フロントでデリヘル利用を断られたり、出禁になったりしませんか？', 'a' => 'カードキー式で直通エレベーターのあるホテルや、来訪者に寛容なホテルなら断られにくい傾向があります。心配な場合は、各ホテルページで入室方法（直通・カードキー・フロント相談）や呼べた実績の口コミを確認しておくと安心です。'],
                ['q' => 'ラブホテルでもデリヘルは呼べますか？', 'a' => 'ラブホテルでも呼べる場合がありますが、基本的に事前予約ができず当日の空室状況によること、二人での利用を前提とした造りのため一人での入室（チェックイン）を受け付けているかの確認が必要です。YobuHoはビジネスホテル・シティホテルを中心に、ラブホテルの一人入室の可否もあわせて確認できます。'],
                ['q' => 'デリヘルを呼べるホテルを事前に確認する方法はありますか？', 'a' => 'YobuHoでエリアやホテル名から検索し、各ホテルの「呼べた／呼べなかった」口コミと店舗の案内実績を確認するのが確実です。フロント対応や入室方法までわかるため、当日の不安を減らせます。'],
            ],
        ],
        'women' => [
            'intro_h2' => '女性用風俗（女風）とは｜サービスの流れとホテルでの利用',
            'intro' => [
                '女性用風俗（女風）は、女性がお気に入りの男性セラピストを指名し、マッサージやリラクゼーションを通じて心身ともに癒やされるサービスです。一般的な流れは、まずカウンセリングで体調や希望を確認し、全身マッサージで体をほぐしていきます。肌にやさしいオイルを使った丁寧な施術で、足裏・ふくらはぎ・背中・首などをじっくりとケアし、十分にリラックスした状態から、女風ならではの性感マッサージへと進んでいきます。',
                'コースは、短時間のお試しから数時間じっくり過ごせるもの、さらにはお気に入りのセラピストと長時間一緒に過ごせる「お泊まりコース」まで、幅広く用意されているのが一般的です。利用場所は自宅とホテルの両方に対応する店舗が多く、待ち合わせてセラピストと一緒に目的地へ向かい、デートのような時間を楽しむこともできます。',
                'ホテルを利用する場合に気になるのが、「そのホテルにセラピストを呼べるのか」「フロントで気まずくないか」という点です。YobuHoでは、実際に女風を利用した女性の口コミと、届出確認済みの女風店舗の案内実績から、ビジネスホテル・シティホテルを中心に女風を呼べるホテルを地域や駅名から探せます。',
            ],
            'tips_h3' => '女風セラピストを呼びやすいホテルの特徴',
            'tips' => [
                ['t' => '女性が一人で利用しやすいホテル', 'd' => 'チェックインや館内の移動で人目が気になりにくいホテルなら、女性一人でも安心して利用できます。'],
                ['t' => '直通エレベーター・カードキー式', 'd' => 'フロントを通らず部屋へ行ける構造なら、セラピストと合流しても気兼ねがありません。'],
                ['t' => '待ち合わせ・来訪に対応しやすい駅近ホテル', 'd' => 'セラピストとの待ち合わせや移動がスムーズで、交通費も抑えやすい立地です。'],
                ['t' => '防音性の高いラブホテル（一人入室可）', 'd' => 'プライバシーや防音を重視するなら、一人での入室を受け付けているラブホテルも選択肢になります。'],
            ],
            'faq' => [
                ['q' => '女性用風俗（女風）はビジネスホテルやシティホテルに呼べますか？', 'a' => '多くのビジネスホテル・シティホテルで女風セラピストの利用は可能です。ただしホテルごとに対応が分かれるため、YobuHoでは利用者の口コミと店舗の案内実績から「呼べた／呼べなかった」を事前に確認できます。'],
                ['q' => 'ビジネスホテルやシティホテルで女風を呼ぶとき、気をつけることはありますか？', 'a' => 'ビジネスホテルやシティホテルは一人でのチェックインが一般的なため、後からセラピストが訪ねてきても基本的に問題なく利用できます。ただしフロントを必ず通る造りのホテルや、来訪者の確認が厳しいホテルもあるため、カードキー式・直通エレベーターなど入室方法を口コミで確認しておくと安心です。'],
                ['q' => 'ラブホテルでも女風は呼べますか？', 'a' => 'ラブホテルでも利用できますが、基本的に事前予約ができず当日の空室状況によること、二人での利用を前提とした造りのため一人での入室（チェックイン）を受け付けているかの確認が必要です。YobuHoはビジネスホテル・シティホテルを中心に、ラブホテルの一人入室の可否もあわせて確認できます。'],
                ['q' => '女風を呼べるホテルを事前に確認する方法はありますか？', 'a' => 'YobuHoでエリアやホテル名から検索し、各ホテルの「呼べた／呼べなかった」口コミと店舗の案内実績を確認するのが確実です。フロント対応や入室方法までわかるため、当日の不安を減らせます。'],
            ],
        ],
        'este' => [
            'intro_h2' => 'デリエステ（風俗エステ・回春マッサージ）を呼べるホテルの選び方',
            'intro' => [
                'デリエステ（デリバリー風俗エステ）は、女性セラピストがホテルや自宅に出張し、オイルを使った回春マッサージや風俗エステ、M性感などの施術を提供するサービスです。マッサージを伴うため、ある程度の広さやシャワー設備が整ったホテルが利用に向いています。ただし、すべてのホテルでセラピストを呼べるわけではなく、フロントで断られるケースもあります。',
                'デリエステを呼びやすいのは、フロントを通らず部屋へ行けるカードキー式・直通エレベーターのホテルや、ベッド以外にも施術スペースを確保しやすいビジネスホテル・シティホテルです。施術ではオイルやタオルを使うため、清潔感やバスルームの使いやすさもチェックポイントになります。',
                'YobuHoでは、実際にデリエステを利用した人の「呼べた／呼べなかった」口コミと、届出確認済みのエステ店舗の案内実績から、ビジネスホテル・シティホテルを中心にデリエステを呼べるホテルを地域や駅名から検索できます。',
            ],
            'tips_h3' => 'デリエステを呼びやすいホテルの特徴',
            'tips' => [
                ['t' => '施術スペースを確保しやすい広めの客室', 'd' => 'オイルマッサージを行うため、ベッド周りにゆとりのある部屋だと施術がスムーズです。'],
                ['t' => 'シャワー・バスルームが使いやすいホテル', 'd' => '施術前後にシャワーを使うことが多く、清潔で使いやすいバスルームがあると快適です。'],
                ['t' => 'カードキー式・直通エレベーター', 'd' => 'フロントを通らず部屋へ直行できるため、セラピストの出入りが目立たず呼びやすい構造です。'],
                ['t' => '駅近で滞在しやすいビジネスホテル', 'd' => '60〜120分など長めの施術でも落ち着いて過ごせ、セラピストの移動・交通費も抑えやすい立地です。'],
            ],
            'faq' => [
                ['q' => 'デリエステ（風俗エステ）はビジネスホテルやシティホテルに呼べますか？', 'a' => '多くのビジネスホテル・シティホテルでデリエステセラピストの利用は可能です。ただしホテルごとに対応が分かれるため、YobuHoでは利用者の「呼べた／呼べなかった」口コミと店舗の案内実績から事前に確認できます。'],
                ['q' => 'オイルマッサージでも問題なく利用できますか？', 'a' => 'オイルやタオルを使う施術が中心のため、施術スペースに余裕があり、シャワー・バスルームが使いやすいホテルだと快適に利用できます。各ホテルページで部屋タイプや設備の口コミを確認しておくと安心です。'],
                ['q' => 'ラブホテルでもデリエステは呼べますか？', 'a' => 'ラブホテルでも利用できますが、基本的に事前予約ができず当日の空室状況によること、二人での利用を前提とした造りのため一人での入室（チェックイン）を受け付けているかの確認が必要です。YobuHoはビジネスホテル・シティホテルを中心に、ラブホテルの一人入室の可否もあわせて確認できます。'],
                ['q' => 'デリエステを呼べるホテルを事前に確認する方法はありますか？', 'a' => 'YobuHoでエリアやホテル名から検索し、各ホテルの「呼べた／呼べなかった」口コミと店舗の案内実績を確認するのが確実です。フロント対応や入室方法までわかるため、当日の不安を減らせます。'],
            ],
        ],
        'men_same' => [
            'intro_h2' => '男性同士（ゲイ風俗・ゲイカップル）で利用できるホテルの選び方',
            'intro' => [
                '男性同士でホテルを利用するシーンには、ゲイ風俗（男性向けの同性風俗）のセラピストを呼ぶ場合と、ゲイカップルが二人で宿泊・デートで利用する場合があります。いずれも「このホテルは男性同士で使えるのか」「フロントで気まずくないか」「呼んだセラピストが来ても、二人で泊まっても大丈夫か」といった不安がつきものです。ホテルによって対応は分かれます。',
                '呼びやすく・利用しやすいのは、フロントを通らず部屋へ行けるカードキー式・直通エレベーターのホテルや、同性ツイン利用が一般的なビジネスホテル・シティホテルです。一方ラブホテルは、店舗によって同性カップルの利用やセラピストの来訪可否が分かれるため、事前の確認が安心です。',
                'YobuHoでは、実際に男性同士で利用した人の「呼べた／呼べなかった」口コミと、同性利用に対応したホテル情報から、ビジネスホテル・シティホテルを中心に男性同士で利用できるホテルを地域や駅名から検索できます。',
            ],
            'tips_h3' => '男性同士で利用しやすいホテルの特徴',
            'tips' => [
                ['t' => '同性ツインで泊まれるビジネス・シティホテル', 'd' => '出張や旅行での同性ツイン利用が一般的で、気兼ねなく宿泊できます。'],
                ['t' => 'カードキー式・直通エレベーター', 'd' => 'フロントを通らず部屋へ行けるため、セラピストの出入りも目立たず、人目を気にせず利用しやすい構造です。'],
                ['t' => '同性利用OKを明示しているラブホテル', 'd' => '同性カップルの利用を受け入れているラブホテルなら、断られる心配がありません。'],
                ['t' => '繁華街・駅近で客層が幅広いホテル', 'd' => 'さまざまな利用者が訪れるエリアのホテルは、同性利用にも寛容な傾向があります。'],
            ],
            'faq' => [
                ['q' => 'ゲイ風俗（男性向け同性風俗）はホテルに呼べますか？', 'a' => '多くのビジネスホテル・シティホテルでゲイ風俗のセラピストを呼ぶことは可能です。ただしホテルごとに対応が分かれるため、YobuHoでは利用者の口コミと店舗の案内実績から「呼べた／呼べなかった」を事前に確認できます。'],
                ['q' => 'ゲイカップル（男性同士）でビジネスホテルやシティホテルに泊まれますか？', 'a' => 'ビジネスホテルやシティホテルは同性同士のツイン利用が一般的で、基本的に問題なく宿泊できます。YobuHoでは利用者の口コミから、より快適に過ごせるホテルを確認できます。'],
                ['q' => 'ラブホテルは男性同士でも利用できますか？', 'a' => 'ラブホテルは店舗によって同性カップルの利用やセラピストの来訪の可否が分かれます。同性利用OKを明示しているホテルや、口コミで実績のあるホテルを選ぶと安心です。YobuHoで事前に確認できます。'],
                ['q' => '男性同士で利用できるホテルを事前に確認する方法はありますか？', 'a' => 'YobuHoでエリアやホテル名から検索し、同性利用に対応したホテルや利用者の口コミを確認するのが確実です。フロント対応や入室方法までわかるため、当日の不安を減らせます。'],
            ],
        ],
        'women_same' => [
            'intro_h2' => '女性同士（レズ風俗・レズビアンカップル）で利用できるホテルの選び方',
            'intro' => [
                '女性同士でホテルを利用するシーンには、レズ風俗（女性向けの同性風俗）のセラピストを呼ぶ場合と、レズビアンカップルが二人で宿泊・デートで利用する場合があります。いずれも「このホテルは女性同士で使えるのか」「フロントで気まずくないか」「呼んだセラピストが来ても、二人で泊まっても大丈夫か」といった不安がつきものです。ホテルによって対応は分かれます。',
                '呼びやすく・利用しやすいのは、フロントを通らず部屋へ行けるカードキー式・直通エレベーターのホテルや、同性ツイン利用が一般的なビジネスホテル・シティホテルです。一方ラブホテルは、店舗によって同性カップルの利用やセラピストの来訪可否が分かれるため、事前の確認が安心です。',
                'YobuHoでは、実際に女性同士で利用した人の「呼べた／呼べなかった」口コミと、同性利用に対応したホテル情報から、ビジネスホテル・シティホテルを中心に女性同士で利用できるホテルを地域や駅名から検索できます。',
            ],
            'tips_h3' => '女性同士で利用しやすいホテルの特徴',
            'tips' => [
                ['t' => '同性ツインで泊まれるビジネス・シティホテル', 'd' => '旅行や出張での同性ツイン利用が一般的で、気兼ねなく宿泊できます。'],
                ['t' => 'カードキー式・直通エレベーター', 'd' => 'フロントを通らず部屋へ行けるため、セラピストの出入りも目立たず、人目を気にせず利用しやすい構造です。'],
                ['t' => '同性利用OKを明示しているラブホテル', 'd' => '同性カップルの利用を受け入れているラブホテルなら、断られる心配がありません。'],
                ['t' => '繁華街・駅近で客層が幅広いホテル', 'd' => 'さまざまな利用者が訪れるエリアのホテルは、同性利用にも寛容な傾向があります。'],
            ],
            'faq' => [
                ['q' => 'レズ風俗（女性向け同性風俗）はホテルに呼べますか？', 'a' => '多くのビジネスホテル・シティホテルでレズ風俗のセラピストを呼ぶことは可能です。ただしホテルごとに対応が分かれるため、YobuHoでは利用者の口コミと店舗の案内実績から「呼べた／呼べなかった」を事前に確認できます。'],
                ['q' => 'レズビアンカップル（女性同士）でビジネスホテルやシティホテルに泊まれますか？', 'a' => 'ビジネスホテルやシティホテルは同性同士のツイン利用が一般的で、基本的に問題なく宿泊できます。YobuHoでは利用者の口コミから、より快適に過ごせるホテルを確認できます。'],
                ['q' => 'ラブホテルは女性同士でも利用できますか？', 'a' => 'ラブホテルは店舗によって同性カップルの利用やセラピストの来訪の可否が分かれます。同性利用OKを明示しているホテルや、口コミで実績のあるホテルを選ぶと安心です。YobuHoで事前に確認できます。'],
                ['q' => '女性同士で利用できるホテルを事前に確認する方法はありますか？', 'a' => 'YobuHoでエリアやホテル名から検索し、同性利用に対応したホテルや利用者の口コミを確認するのが確実です。フロント対応や入室方法までわかるため、当日の不安を減らせます。'],
            ],
        ],
    ];
    ...
    $genreSec = '';
    $faqJsonLd = '';
    if (isset($MODE_TOP_CONTENT[$mode])) {
        $gc = $MODE_TOP_CONTENT[$mode];
        $genreSec .= '<section class="seo-static-content seo-genre-content" style="background:#fff; padding:32px 16px; margin-top:24px; border-top:1px solid #e8d8c8; font-size:14px; line-height:1.95; color:#3a2a1f;">';
        $genreSec .= '<div style="max-width:900px; margin:0 auto;">';
        if (!empty($gc['intro_h2'])) {
            $genreSec .= '<h2 style="font-size:18px; margin:0 0 14px; color:' . $topAccent . '; border-left:4px solid ' . $topAccent . '; padding-left:10px;">' . $topEsc($gc['intro_h2']) . '</h2>';
        }
        foreach ($gc['intro'] ?? [] as $p) {
            $genreSec .= '<p style="margin:0 0 14px;">' . $topEsc($p) . '</p>';
        }
        if (!empty($gc['tips'])) {
            $genreSec .= '<h3 style="font-size:16px; margin:24px 0 12px; color:' . $topAccent . '; border-left:4px solid ' . $topAccent . '; padding-left:10px;">' . $topEsc($gc['tips_h3'] ?? '') . '</h3>';
            $genreSec .= '<ul style="list-style:none; padding:0; margin:0 0 8px; display:grid; gap:10px;">';
            foreach ($gc['tips'] as $tip) {
                $genreSec .= '<li style="background:#faf6f0; border:1px solid #e8d8c8; border-radius:8px; padding:12px 14px;"><strong style="color:' . $topAccent . ';">' . $topEsc($tip['t']) . '</strong><br><span style="color:#5a4a3a;">' . $topEsc($tip['d']) . '</span></li>';
            }
            $genreSec .= '</ul>';
        }
        if (!empty($gc['faq'])) {
            $genreSec .= '<h3 style="font-size:16px; margin:24px 0 12px; color:' . $topAccent . '; border-left:4px solid ' . $topAccent . '; padding-left:10px;">よくある質問</h3>';
            $faqItems = [];
            foreach ($gc['faq'] as $qa) {
                $genreSec .= '<div style="margin:0 0 14px;">';
                $genreSec .= '<p style="margin:0 0 6px; font-weight:600; color:#3a2a1f;">Q. ' . $topEsc($qa['q']) . '</p>';
                $genreSec .= '<p style="margin:0; color:#5a4a3a;">A. ' . $topEsc($qa['a']) . '</p>';
                $genreSec .= '</div>';
                $faqItems[] = [
                    '@type' => 'Question',
                    'name' => $qa['q'],
        ...
        $genreSec .= '</div></section>';
    }

    // 独自ジャンルコンテンツ → 47都道府県グリッドの順で <main> 末尾に注入
    $html = str_replace('</main>', $genreSec . $sec . '</main>', $html);
    if ($faqJsonLd !== '') {
        $html = str_replace('</head>', $faqJsonLd . '</head>', $html);
    }
...
if ($hotel_id) {
    ...
    if ($shopData) {
        ...
        $seo_desc  = $sLoc
            ? "{$sn}は{$sLoc}を拠点に{$m['label']}を派遣する店舗です。対応エリア・複数の拠点情報に加え、利用者の口コミ、ホテルへの案内実績、呼べる対応可能なホテル一覧を掲載。安心して利用できるホテル選びをサポートします。"
            : "{$sn}は{$m['label']}を派遣する店舗です。利用者の口コミ、ホテルへの案内実績、呼べる対応可能なホテル一覧を掲載し、安心して利用できるホテル選びをサポートします。";
