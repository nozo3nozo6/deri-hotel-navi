...
(function(){
  ...
  // 立川市を起点に、近い順に並べる市区町村（出張エリア。立川が最多のため先頭）
  const CITY_PROXIMITY = [
    '立川市',
    '国立市', '昭島市', '日野市', '国分寺市', '小平市',
    '府中市', '八王子市', '小金井市', '多摩市', '福生市', '東村山市',
    '羽村市', '稲城市', '東久留米市', '三鷹市', 'あきる野市', '西多摩郡瑞穂町',
    '調布市', '武蔵野市', '西東京市', '青梅市',
    '町田市', '西多摩郡檜原村', '西多摩郡奥多摩町',
  ];
  ...
  // ========== Treatment Menus (施術メニューマスタ) ==========
  let treatmentMenusCache = [];
  let editingTreatmentMenuId = null;
  async function loadTreatmentMenus() {
    const el = document.getElementById('treatmentMenuList');
    if (!el) return;
    try {
      const d = await api('/treatment-menus.php?action=list&include_inactive=1');
      treatmentMenusCache = d.menus || [];
      renderTreatmentMenus();
    } catch (e) {
      el.innerHTML = '<div class="view-empty">読み込み失敗</div>';
    }
  }
  function renderTreatmentMenus() {
    const el = document.getElementById('treatmentMenuList');
    if (!el) return;
    if (treatmentMenusCache.length === 0) {
      el.innerHTML = '<div class="view-empty">施術メニューがありません</div>';
      return;
    }
    el.innerHTML = treatmentMenusCache.map(m => {
      const thumb = m.bg_image_url
        ? `<img src="${escapeHtml(m.bg_image_url)}?t=${Date.now()}" style="width:64px;height:48px;object-fit:cover;border-radius:6px;border:1px solid var(--gray);">`
        : `<div style="width:64px;height:48px;background:var(--foam);border-radius:6px;border:1px dashed var(--gray);display:flex;align-items:center;justify-content:center;font-size:.7rem;color:var(--ink-soft);">画像なし</div>`;
      const inactive = Number(m.is_active) === 0 ? '<span class="hint" style="color:var(--red);">非表示</span>' : '';
      return `<div class="bk-row" data-tm-edit="${m.id}" style="grid-template-columns:auto 1fr auto;cursor:pointer;">
        ${thumb}
        <div><div class="bi-name">${escapeHtml(m.name)} ${inactive}</div><div class="bi-meta">${escapeHtml(m.guide || '')}</div></div>
        <button class="btn-secondary" style="padding:.4rem .8rem;font-size:.85rem;">編集</button>
      </div>`;
    }).join('');
    el.querySelectorAll('[data-tm-edit]').forEach(row => {
      row.addEventListener('click', () => openTreatmentMenuModal(Number(row.dataset.tmEdit)));
    });
  }
  function openTreatmentMenuModal(id) {
    const m = treatmentMenusCache.find(x => x.id === id);
    if (!m) return;
    editingTreatmentMenuId = id;
    document.getElementById('tmName').value = m.name || '';
    document.getElementById('tmDescription').value = m.description || '';
    document.getElementById('tmGuide').value = m.guide || '';
    document.getElementById('tmIsActive').checked = Number(m.is_active) === 1;
    setTmBgPreview(m.bg_image_url || '');
    openModal('treatmentMenuModal');
  }
  function setTmBgPreview(url) {
    const preview = document.getElementById('tmBgPreview');
    const fileInput = document.getElementById('tmBgFile');
    const status = document.getElementById('tmBgStatus');
    if (url) {
      document.getElementById('tmBgPreviewImg').src = url + '?t=' + Date.now();
      preview.style.display = '';
      fileInput.style.display = 'none';
    } else {
      preview.style.display = 'none';
  ...
  async function uploadTmBg(file) {
    if (!editingTreatmentMenuId) return;
    const status = document.getElementById('tmBgStatus');
    status.textContent = 'アップロード中...'; status.style.color = '';
    const fd = new FormData();
    fd.append('menu_id', String(editingTreatmentMenuId));
    fd.append('image', file);
    try {
      const res = await fetch('/ctrl/ops/api/treatment-menus-upload.php', { method: 'POST', body: fd, credentials: 'include' });
      const d = await res.json();
      if (!d.ok) throw new Error(d.error || 'upload failed');
      const m = treatmentMenusCache.find(x => x.id === editingTreatmentMenuId);
      if (m) m.bg_image_url = d.url;
      setTmBgPreview(d.url);
      status.textContent = `✓ アップロード完了 (${Math.round(d.size / 1024)} KB)`;
      status.style.color = 'var(--green)';
      renderTreatmentMenus();
    } catch (e) {
  ...
  async function deleteTmBg() {
    if (!editingTreatmentMenuId || !confirm('背景画像を削除しますか？')) return;
    try {
      await apiPost('/treatment-menus.php?action=update', { id: editingTreatmentMenuId, bg_image_url: null });
      const m = treatmentMenusCache.find(x => x.id === editingTreatmentMenuId);
      if (m) m.bg_image_url = null;
      setTmBgPreview('');
      renderTreatmentMenus();
      toast('✓ 削除しました', 'ok');
    } catch (e) { toast('削除失敗: ' + e.message, 'err'); }
  }
  async function saveTreatmentMenu() {
    if (!editingTreatmentMenuId) return;
    const payload = {
      id: editingTreatmentMenuId,
      name: document.getElementById('tmName').value.trim(),
      description: document.getElementById('tmDescription').value.trim(),
      guide: document.getElementById('tmGuide').value.trim(),
      is_active: document.getElementById('tmIsActive').checked ? 1 : 0,
    };
    if (!payload.name) { toast('メニュー名を入力してください', 'err'); return; }
    try {
      await apiPost('/treatment-menus.php?action=update', payload);
      toast('✓ 保存しました', 'ok');
      closeModal('treatmentMenuModal');
      await loadTreatmentMenus();
    } catch (e) { toast('保存失敗: ' + e.message, 'err'); }
  }
  ...
  const CP_STATUS = { current: ['表示中', '#2e9e5b'], scheduled: ['開始前', '#b8860b'], ended: ['終了', '#8a97a0'], inactive: ['無効', '#c0392b'] };

  async function loadCampaigns() {
    const el = document.getElementById('campaignList');
    if (!el) return;
    try {
      const d = await api('/campaigns.php?action=list');
      campaignsCache = d.campaigns || [];
      renderCampaigns();
    } catch (e) { el.innerHTML = '<div class="view-empty">読み込み失敗</div>'; }
  }
  function fmtCampRange(c) {
    const f = (s) => s ? String(s).slice(0, 16).replace('T', ' ') : '';
  ...
  function renderCampaigns() {
    const el = document.getElementById('campaignList');
    if (!el) return;
    if (!campaignsCache.length) { el.innerHTML = '<div class="view-empty">キャンペーンがありません。「+ キャンペーン追加」から作成できます。</div>'; return; }
    el.innerHTML = campaignsCache.map(c => {
      const [label, color] = CP_STATUS[c.status] || ['—', '#8a97a0'];
      return `<div class="bk-row" data-cp-edit="${c.id}" style="grid-template-columns:auto 1fr auto;cursor:pointer;">
        <span style="background:${color};color:#fff;font-size:.7rem;font-weight:700;padding:.15rem .55rem;border-radius:50px;white-space:nowrap;">${label}</span>
        <div><div class="bi-name">${escapeHtml(c.tag_label || '(タグなし)')} ・ ${escapeHtml(c.headline || c.banner_strong || '(見出しなし)')}</div><div class="bi-meta">${escapeHtml(fmtCampRange(c))}</div></div>
        <button class="btn-secondary" style="padding:.4rem .8rem;font-size:.85rem;">編集</button>
      </div>`;
    }).join('');
    el.querySelectorAll('[data-cp-edit]').forEach(row => row.addEventListener('click', () => openCampaignModal(Number(row.dataset.cpEdit))));
  }
  function cpFieldVals() {
    return {
      tag_label: document.getElementById('cpTag').value.trim(),
      pill_label: document.getElementById('cpPill').value.trim(),
      marquee_text: document.getElementById('cpMarquee').value.trim(),
      banner_strong: document.getElementById('cpStrong').value.trim(),
      headline: document.getElementById('cpHeadline').value.trim(),
      headline_small: document.getElementById('cpHeadlineSmall').value.trim(),
      note: document.getElementById('cpNote').value.trim(),
      starts_at: document.getElementById('cpStart').value ? document.getElementById('cpStart').value.replace('T', ' ') + ':00' : '',
      ends_at: document.getElementById('cpEnd').value ? document.getElementById('cpEnd').value.replace('T', ' ') + ':00' : '',
      is_active: document.getElementById('cpActive').checked ? 1 : 0,
    };
  }
  function renderCampaignPreview() {
    const p = document.getElementById('cpPreview');
    if (!p) return;
    const v = cpFieldVals();
    const brs = (s) => escapeHtml(s).replace(/\r?\n/g, '<br>');  // 改行を<br>に（公開バナーと同じ）
    p.innerHTML = `${v.tag_label ? `<span class="op-tag">${escapeHtml(v.tag_label)}</span>` : ''}
      ${v.banner_strong ? `<strong>${brs(v.banner_strong)}</strong>` : ''}
      ${v.headline ? `<span class="op-headline">${brs(v.headline)}${v.headline_small ? `<small>${brs(v.headline_small)}</small>` : ''}</span>` : ''}
      ${v.note ? `<span class="banner-note">${brs(v.note)}</span>` : ''}`;
  }
  function openCampaignModal(id) {
    editingCampaignId = id || null;
    const c = id ? campaignsCache.find(x => Number(x.id) === Number(id)) : null;
    const toLocal = (s) => s ? String(s).slice(0, 16).replace(' ', 'T') : '';
    document.getElementById('cpTitle').textContent = c ? 'キャンペーン編集' : '新規キャンペーン';
    document.getElementById('cpTag').value = c ? (c.tag_label || '') : '';
    document.getElementById('cpPill').value = c ? (c.pill_label || '') : '';
    document.getElementById('cpMarquee').value = c ? (c.marquee_text || '') : '';
    document.getElementById('cpStrong').value = c ? (c.banner_strong || '') : '';
    document.getElementById('cpHeadline').value = c ? (c.headline || '') : '';
    document.getElementById('cpHeadlineSmall').value = c ? (c.headline_small || '') : '';
    document.getElementById('cpNote').value = c ? (c.note || '') : '';
    document.getElementById('cpStart').value = c ? toLocal(c.starts_at) : '';
    document.getElementById('cpEnd').value = c ? toLocal(c.ends_at) : '';
    document.getElementById('cpActive').checked = c ? Number(c.is_active) === 1 : true;
    document.getElementById('cpDelete').style.display = c ? '' : 'none';
    renderCampaignPreview();
    openModal('campaignModal');
  }
  async function saveCampaign() {
    const v = cpFieldVals();
    if (!v.tag_label && !v.marquee_text && !v.banner_strong && !v.headline) { toast('タグ・マーキー・見出しのいずれかを入力してください', 'err'); return; }
    if (v.starts_at && v.ends_at && v.starts_at > v.ends_at) { toast('終了日時は開始日時より後にしてください', 'err'); return; }
    const payload = editingCampaignId ? { id: editingCampaignId, ...v } : v;
    try {
      await apiPost('/campaigns.php?action=save', payload);
      toast('✓ 保存しました', 'ok');
  ...
  async function deleteCampaign() {
    ...
    try {
      await apiPost('/campaigns.php?action=delete', { id: editingCampaignId });
      toast('削除しました', 'ok');
      closeModal('campaignModal');
      await loadCampaigns();
    } catch (e) { toast('削除失敗: ' + e.message, 'err'); }
  ...
  function setupEntryMethodSortable() {
    ...
    list.querySelectorAll('.sortable').forEach(row => {
      ...
      row.addEventListener('drop', async e => {
        ...
        list.insertBefore(dragSrc, before ? row : row.nextSibling);
        list.querySelectorAll('.drag-over-top, .drag-over-bottom').forEach(r => r.classList.remove('drag-over-top', 'drag-over-bottom'));
        const newIds = Array.from(list.querySelectorAll('.sortable')).map(r => Number(r.dataset.emId));
        try {
          await apiPost('/entry-methods.php?action=reorder', { ids: newIds });
          entryMethodsCache.sort((a, b) => newIds.indexOf(a.id) - newIds.indexOf(b.id));
          entryMethodsCache.forEach((e, i) => e.sort_order = (i + 1) * 10);
          populateEntryMethodSelect();
          toast('✓ 順序を更新', 'ok');
        } catch (err) {
          toast('順序更新失敗: ' + err.message, 'err');
  ...
  async function init() {
    ...
    // キャンペーンマスタ
    document.getElementById('cpAddNew')?.addEventListener('click', () => openCampaignModal(null));
    document.getElementById('cpSave')?.addEventListener('click', saveCampaign);
    document.getElementById('cpDelete')?.addEventListener('click', deleteCampaign);
    ['cpTag', 'cpMarquee', 'cpStrong', 'cpHeadline', 'cpHeadlineSmall', 'cpNote'].forEach(id => {
      document.getElementById(id)?.addEventListener('input', renderCampaignPreview);
    });
    // 施術メニューマスタ
    document.getElementById('tmSave').addEventListener('click', saveTreatmentMenu);
    document.getElementById('tmBgFile').addEventListener('change', e => {
      const f = e.target.files && e.target.files[0];
      if (f) uploadTmBg(f);
    });
    document.getElementById('tmBgReplace').addEventListener('click', () => document.getElementById('tmBgFile').click());
    document.getElementById('tmBgDelete').addEventListener('click', deleteTmBg);
    // 背景画像アップロード
