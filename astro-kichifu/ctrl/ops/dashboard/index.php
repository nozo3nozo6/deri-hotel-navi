...
<div class="view" id="view-courses">
  ...
  <!-- 施術メニューマスタ (トップページの「施術メニュー」カードを編集) -->
  <div class="tl-toolbar" style="margin-top:2rem;">
    <div class="tl-title">💆 施術メニューマスタ<span class="tl-sub">トップページに表示される 6 種のメニューカードと背景画像を編集</span></div>
  </div>
  <div class="staff-main" style="max-width:900px;">
    <div id="treatmentMenuList" class="bk-list"><div class="loading"><span class="spinner"></span><br><br>読み込み中...</div></div>
  </div>

  <!-- 施術メニュー編集モーダル -->
  <div class="modal-overlay" id="treatmentMenuModal">
    <div class="modal" style="max-width:520px;">
      <div class="modal-header">
        <div><div class="mh-title">施術メニュー編集</div></div>
        <button class="modal-close" data-close="treatmentMenuModal">×</button>
      </div>
      <div class="modal-body">
        <div class="field"><label for="tmName">メニュー名</label><input type="text" id="tmName"></div>
        <div class="field"><label for="tmDescription">説明文</label><textarea id="tmDescription" rows="3"></textarea></div>
        <div class="field"><label for="tmGuide">目安表示</label><input type="text" id="tmGuide" placeholder="例: 目安: 20分〜 / 全身40分〜"></div>
        <div class="field">
          <label>背景画像 <span class="hint" style="font-weight:400;">推奨 1600×1200 (4:3)、自動 WebP 化</span></label>
          <div id="tmBgPreview" style="margin-top:.4rem;display:none;">
            <img id="tmBgPreviewImg" style="width:100%;max-width:280px;aspect-ratio:4/3;object-fit:cover;border-radius:8px;border:1px solid var(--gray);">
            <div style="margin-top:.4rem;display:flex;gap:.5rem;">
              <button type="button" class="btn-secondary" id="tmBgReplace" style="padding:.4rem .8rem;font-size:.85rem;">差し替え</button>
              <button type="button" class="btn-secondary" id="tmBgDelete" style="padding:.4rem .8rem;font-size:.85rem;color:var(--red);border-color:var(--red);">削除</button>
            </div>
          </div>
          <input type="file" id="tmBgFile" accept="image/jpeg,image/png,image/webp" style="margin-top:.4rem;">
          <div id="tmBgStatus" style="margin-top:.4rem;font-size:.82rem;color:var(--ink-soft);"></div>
        </div>
        <div class="field"><label><input type="checkbox" id="tmIsActive" checked style="width:auto;margin-right:.4rem;"> このメニューをトップに表示</label></div>
      </div>
      <div class="modal-footer">
        <button class="btn-secondary" data-close="treatmentMenuModal">キャンセル</button>
        <button class="btn-primary" id="tmSave">保存</button>
      </div>
    </div>
  </div>

  <!-- 駅マスタ（基本交通費）→ 独立ビューへ移動。ここは入口のみ -->
  <div class="tl-toolbar" style="margin-top:2rem;">
    <div class="tl-title">🚉 駅マスタ・基本交通費<span class="tl-sub">各駅の基本交通費を設定（出張費シミュレーション用）</span></div>
    <div class="tl-nav">
      <button class="tl-add" id="openStationMgr" type="button">駅マスタを開く →</button>
    </div>
  </div>

  <!-- 🎉 キャンペーン（イベント）マスタ -->
  <div class="tl-toolbar" style="margin-top:2rem;">
    <div class="tl-title">🎉 キャンペーンマスタ<span class="tl-sub">トップのマーキー＋料金バナーを管理（期間・有効で自動表示、デプロイ不要）</span></div>
    <div class="tl-nav">
      <button class="tl-add" id="cpAddNew" type="button" style="background:var(--coral);color:#fff;border:none;padding:.55rem 1.3rem;border-radius:50px;font-size:.88rem;font-weight:600;cursor:pointer;">+ キャンペーン追加</button>
    </div>
  </div>
  <div class="staff-main" style="max-width:900px;">
    <div id="campaignList" class="bk-list"><div class="loading"><span class="spinner"></span><br><br>読み込み中...</div></div>
  </div>
  ...
  <div class="modal-overlay" id="campaignModal">
    <div class="modal" style="max-width:560px;">
      <div class="modal-header">
        <div><div class="mh-title" id="cpTitle">新規キャンペーン</div></div>
        <button class="modal-close" data-close="campaignModal">×</button>
      </div>
      <div class="modal-body">
        <div class="field"><label for="cpTag">タグ<span class="hint" style="font-weight:400;"> バナー左上の英字ラベル（例: TRAVEL SUPPORT）</span></label><input type="text" id="cpTag" placeholder="TRAVEL SUPPORT"></div>
        <div class="field"><label for="cpPill">切替タブ名<span class="hint" style="font-weight:400;"> 複数キャンペーン時に下部タブへ表示（例: ホテルでご利用）</span></label><input type="text" id="cpPill" placeholder="ホテルでご利用"></div>
        <div class="field"><label for="cpMarquee">マーキー文<span class="hint" style="font-weight:400;"> 上部を流れるテロップ</span></label><input type="text" id="cpMarquee" placeholder="ご予約時にこちらからご優待をご案内します"></div>
        <div class="field"><label for="cpStrong">バナー見出し（強調）<span class="hint" style="font-weight:400;"> バナー上段の太字・改行=Enter</span></label><textarea id="cpStrong" rows="2" placeholder="ご予約時に、こちらから特別ご優待をご案内します"></textarea></div>
        <div class="field"><label for="cpHeadline">キャンペーン名（大見出し）<span class="hint" style="font-weight:400;"> 改行=Enter</span></label><textarea id="cpHeadline" rows="2" placeholder="出張応援キャンペーン。特別ご優待開催中"></textarea></div>
        <div class="field"><label for="cpHeadlineSmall">対象・期限（小文字）<span class="hint" style="font-weight:400;"> 改行=Enter</span></label><textarea id="cpHeadlineSmall" rows="2" placeholder="ホテルご利用のお客様限定〜7月19日(日)まで"></textarea></div>
        <div class="field"><label for="cpNote">注記<span class="hint" style="font-weight:400;"> ※から始まる補足</span></label><textarea id="cpNote" rows="2" placeholder="※お客様からのお申し出は不要です。他媒体・広告掲載サイトは…"></textarea></div>
        <div style="display:flex;gap:.6rem;">
          <div class="field" style="flex:1;"><label for="cpStart">開始日時<span class="hint" style="font-weight:400;"> 空=即時</span></label><input type="datetime-local" id="cpStart"></div>
          <div class="field" style="flex:1;"><label for="cpEnd">終了日時<span class="hint" style="font-weight:400;"> 空=無期限</span></label><input type="datetime-local" id="cpEnd"></div>
        </div>
        <div class="field"><label><input type="checkbox" id="cpActive" checked style="width:auto;margin-right:.4rem;"> 有効にする（期間内なら表示）</label></div>
        <div style="margin-top:.6rem;padding:.7rem;background:var(--foam);border-radius:10px;">
          <div style="font-size:.78rem;color:var(--ink-soft);margin-bottom:.4rem;">プレビュー（バナー）</div>
          <div class="opening-banner" id="cpPreview" style="margin:0;"></div>
        </div>
      </div>
      <div class="modal-footer">
        <button class="btn-secondary" id="cpDelete" type="button" style="margin-right:auto;color:var(--red);border-color:var(--red);display:none;">削除</button>
        <button class="btn-secondary" data-close="campaignModal">キャンセル</button>
        <button class="btn-primary" id="cpSave">保存</button>
      </div>
    </div>
  </div>
</div>
