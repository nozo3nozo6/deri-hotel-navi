...
// 一覧は「今見ている店のサイトに出るもの」＋「自店が作った表示先なしの行」。
// owner で絞ると他店が作った行が見えず、サイトには出続ける（display_scope_sql の説明を参照）。
[$scope, $bind] = display_scope_sql('s', 'slider_shops', 'slider_id', $shop);
$st = db()->prepare("SELECT * FROM sliders s WHERE $scope ORDER BY s.sort, s.id");
$st->execute($bind);
$rows = $st->fetchAll();
...
$shopName = '';
foreach (shops_list() as $sh) if ((int)$sh['id'] === $shop) $shopName = (string)$sh['area'];
...
<p class="muted" style="margin:-6px 0 12px"><?= h($shopName) ?>のサイトに出るスライダーです（他店が作ったものもここに出ます）。行をドラッグで並べ替え</p>
    ...
    <tbody data-sortable>
      <?php foreach ($rows as $s): $disp = $dispMap[(int)$s['id']] ?? []; ?>
        <tr draggable="true" data-id="<?= (int)$s['id'] ?>">
          ...
          <?php
            // この店に実際に適用されるリンク（店舗別URLがあればそちら）
            $ownUrl = $urlMap[(int)$s['id']][$shop] ?? '';
            $effUrl = $ownUrl !== '' ? $ownUrl : (string)$s['url'];
          ?>
          <td class="muted" style="max-width:240px"><div class="url-cell"><?= h($effUrl) ?></div>
            <?php if ($ownUrl !== ''): ?><span class="url-note">この店だけのリンク</span><?php endif; ?></td>
...
.url-note{display:inline-block;margin-top:2px;font-size:10px;font-weight:700;color:#be185d;background:#fdf2f8;border:1px solid #fbcfe8;border-radius:999px;padding:1px 7px}
</style>
