...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    $upsert = db()->prepare(
        'INSERT INTO play_availability (shop_id, girl_id, play_at, reception_closed, status, updated_by)
         VALUES (?,?,?,0,"active",?)
    ...
    if ($action === 'set') {
        ...
        $st->execute([$shop, $gid, $by]);
        flash('ok', $gname . ': 受付終了にしました（出勤はそのまま／即ヒメ・接客・待機のみ停止。ヒメ割は掲載継続）。');
        media_webhook_notify($shop, $gid, $gname, ['play_at', 'status', 'reception_closed'], 'ctrl',
            ['sugu_hime', 'ekichika', 'heaven', 'fuzoku', 'deli']);
    } elseif ($action === 'clear') {
        // 即姫クリア（受付終了フラグも解除＝cleared と受付終了が同時に立つ曖昧な状態を作らない）
        $st = db()->prepare('UPDATE play_availability SET status="cleared", reception_closed=0, updated_by=? WHERE shop_id=? AND girl_id=?');
        $st->execute([$by, $shop, $gid]);
        flash('ok', $gname . ': クリアしました（媒体側は bot が取消します）。');
        media_webhook_notify($shop, $gid, $gname, ['play_at', 'status', 'reception_closed']);
...
$rows = db()->prepare(
    'SELECT g.id, g.name,
            pa.play_at, pa.reception_closed, pa.shift_end_at, pa.himewari_enabled, pa.himewari_minutes, pa.himewari_price,
  ...
  <tr>
    ...
    <td>
      <div class="pa-forms">
        ...
        <?php if ($g['work_start'] && !$rcClosed): /* 出勤中のみ。受付終了＝出勤は残して即ヒメ系だけ止める（退勤・出勤クリアとは別） */ ?>
        <form method="post">
          <?= csrf_field() ?>
