...
$rows = db()->prepare('SELECT id, name FROM girl_image_tags WHERE shop_id=? ORDER BY sort, id');
$rows->execute([$shop]);
$rows = $rows->fetchAll();
...
<p class="muted" style="margin-top:-8px">女の子に付ける特徴タグ（可愛い系・清楚・スレンダー・愛嬌抜群 など）。女性の編集画面で選択でき、サイトの名前下に表示されます。</p>
...
<div class="table-wrap">
  <table class="tbl">
    <thead><tr><th style="width:34px"></th><th>タグ名</th><th style="width:60px">操作</th></tr></thead>
    <tbody data-sortable id="rows">
      ...
      <?php endforeach; ?>
      <?php if (!$rows): ?><tr><td colspan="3" class="muted" style="text-align:center;padding:30px">タグがありません。上のフォームから追加してください。</td></tr><?php endif; ?>
    </tbody>
  </table>
</div>
