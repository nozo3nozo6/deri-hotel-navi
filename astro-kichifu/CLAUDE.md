...
## 実装履歴（2026-08-03 その2）— スライダーは「全店共通の1本」に統一（店長判断）

同日の一次対応（下記その1）で一覧を表示先基準にしたが、店長から「アドミの方で吉祥寺のON/OFF、でも吉祥寺は吉祥寺で、が中途半端。どちらかにしたい」との指摘。**共有1本**に決定。
- **決定**: スライダーは店舗に依存しない1本の一覧。出し先は各行の「表示店舗」トグルだけで決まる（ヘッダーの店舗ドロップダウンはスライダー画面の一覧を変えない）。同じバナーを1回の登録で両店に出せ、差し替えも1回で済む＝「片方だけ古いまま」が構造的に起きない。
- **実装**: `ctrl/sliders.php` はスコープ無しの `SELECT * FROM sliders ORDER BY sort, id`、`ctrl/slider-edit.php` は id のみで取得、`ctrl/content-actions.php` の `sliders` スコープは `false`。一覧上部に「立川のサイトに出る N枚／吉祥寺のサイトに出る N枚」を常時表示し、リンク列は共通URL＋店舗別URLを店名タグ付きで併記。
- **バナーは対象外**: 店ごとに別レコード運用（吉祥寺高収入求人案内など内容が違う）なので、表示先基準のスコープ（その1）のまま。
- ⚠️ **同一画像を複数行が参照する**: 「高収入求人」は立川行と吉祥寺行が同じ `/uploads/sliders/1/45e8aaadb1d1a291.webp` を指していた。片方を削除して実体まで消すと残った行が画像404になる（2026-07-23 の応急対応で起きた事故と同型）。`ctrl/_upload.php` に `UPLOAD_IMAGE_REFS` / `upload_in_use()` / `delete_upload_safe()` を追加し、**sliders/banners の削除・差し替えは必ず `delete_upload_safe()` 経由**にした。news/girls は既存の所有者ガード（`$isNewsOwn`/`$isOwn`）があるため据え置き。
- **データ整理**: 重複5行を削除（id2/16=立川に残っていたランキングの複製・表示先なし、id5=7/31までの旧イベント、id7=画像404のドライバー募集、id8=高収入求人の重複）。id8 の画像は id4 と同一実体のため**実体は残した**（ガードが正しく判定）。残り5行の sort を 0..4 に振り直し。
- **結果**: 一覧5行 → 立川3枚 / 吉祥寺5枚。API・静的HTMLとも配信内容は整理前と同一（OFF行を消しただけ）なので再ビルド不要。

## 実装履歴（2026-08-03 その1）— CTRL一覧を「そのサイトに出るもの」基準に＋スライダーの店舗別リンクURL

2026-07-23 の表示店舗トグル導入で残っていた穴の後始末。店長報告「kichifu だけ、チェックしたバナーと以前のものが両方出る」。
- **原因**: 一覧クエリがオーナー店(`shop_id`)基準のままだった（上記 2026-07-23 の項に「一覧クエリはオーナー店基準のまま」と明記した箇所）。立川ビューでは吉祥寺owner行が見えないのに、それらは吉祥寺サイトへ配信され続ける。実データで**吉祥寺は8枚配信**（立川owner・両店の3枚＋吉祥寺owner5枚＝イベント/高収入求人/ドライバー募集が二重）。立川は3枚で正常。
- **対応（`ctrl/_lib.php` に `display_scope_sql()` を新設し3箇所で共用）**:
  - 一覧 `ctrl/sliders.php`・`ctrl/banners.php`、編集 `ctrl/slider-edit.php`・`ctrl/banner-edit.php`、非同期操作 `ctrl/content-actions.php`（toggle/delete/reorder）のスコープを **「自店に表示される行」＋「自店が作った表示先なしの行」** に統一。＝一覧に出るもの＝サイトに出るもの。他店が作った行もその店のビューから編集・削除・並べ替えできる。
  - 他店owner行を編集しても `shop_id`（作った店の記録）は移さない。新規スライダーの `sort` は**全店通し**の最大値+1（owner別採番だと番号が衝突し並べ替えが効かない ← 2026-07-23 の⚠️ソート注意の解消）。
  - `content-actions.php` の `$TABLES` 第3要素は `true`（自店のみ）/`false`（共有マスタ）/`[中間テーブル, 外部キー]`（表示先基準）の3値。テーブル名は従来どおりホワイトリスト由来のみ。
- **スライダーの店舗別リンクURL**（`sql/migration_slider_shop_url.sql`、適用済み）: `slider_shops.url` 追加。編集画面の表示店舗トグルの隣にその店専用のリンク欄。空なら共通 `sliders.url`。配信は `COALESCE(NULLIF(ss.url,''), s.url)`。一覧のリンク列はその店に効く実効URLを出し、店舗別のときは「この店だけのリンク」バッジ。※バナーは店ごとに別レコード運用なので未対応（必要になれば `banner_shops.url` を同型で追加）。
- **本番検証**: 立川ビュー=5行（従来と同じ）／吉祥寺ビュー=8行でサイトの8枚と一致／配信API 立川3枚・吉祥寺8枚（無回帰）。
- **残（データ整理・店長判断）**: 吉祥寺の重複 id=8 高収入求人（id=4 と同一画像ファイル）、id=7 ドライバー募集（画像404のリンク切れ）、id=5 イベント（id=1 とは別画像）。id=6/17 ランキングは吉祥寺固有なので残す。
...
## 実装履歴（2026-07-13 深夜2）— CTRLキャッシュ無効化＋出勤終了後の陳腐化表示を修正（コミット 53846a0e）

「ゆあの即姫クリアが効かない」報告の調査。原因2つ:
1. **CTRL全ページにCache-Controlヘッダーが無かった**。`ctrl/_lib.php` `layout_header()` 冒頭に `no-store/no-cache` + `Pragma: no-cache` を追加（全CTRLページに波及）。モバイルSafari等がPOST後のredirect先(GET)を積極キャッシュし、保存/クリア直後に古い表示が残ることがあった。DBレベルではUPDATE自体は正常動作すると直接検証済み（rowCount=1で反映）。
2. **（本命）出勤終了を過ぎても play-availability 一覧が古い play_at のまま表示され続ける境界バグ**。当初 `schedules.end_time`（本日bizDateのみJOIN）で終了判定しようとしたが、**朝5時の営業日切替直後は前日分の出勤がJOINにヒットしなくなり判定が丸ごとスキップされる**ことが受け入れテストの営業日境界ケースで発覚。→ `play_availability.shift_end_at`（P3で出勤保存のたびに自動同期される永続カラム、営業日境界の影響を受けない）を使うよう修正。出勤終了を過ぎたらプレビュー「―」＋時刻セレクト`--:--`（DBのplay_at自体は変更しない・表示のみ）。
3. クリアボタンに確認ダイアログ追加（隣の時刻設定/今すぐボタンとの誤タップ対策）。
- 本番DBレベル検証5項目 ALL PASS（終了後=―／終了前=継続表示／出勤なし=通常動作／**営業日境界を跨いだ陳腐化データ=正しく―**／掃除確認）。
- ⚠️ 教訓: 「本日の出勤情報」を参照する場所が複数ある場合（一覧表示・API等）、`schedules` を都度JOINするより **P3で同期済みの永続コピー（shift_start_at/shift_end_at）を使う方が営業日境界に強い**。P5のAPI GETは`schedules`直接導出のままで正しい（cast_idクエリ都度なので境界の影響を受けにくい設計だが、同種の懸念があれば同じ修正パターンを検討）。

## 実装履歴（2026-07-13 深夜）— P5出勤帯API＋P6ヒメ割簡素化（コミット 8b4bb6a7）

`CLAUDE-SCHEDULE-API.md`（P5）/ `CLAUDE-HIMEWARI-AUTO.md`（P6）準拠。
- **P5**: GETに `shift_start_at`/`shift_end_at`/`shift_business_date` 追加。**値は本日営業日（5時区切り）の schedules(work) から直接導出＝出勤表が正**（保存カラムの写しではない→事前登録/削除/休みが常に正確）。休み・出勤なし→null。**本日出勤があるキャストは play_availability 行が無くてもGETに出る**（play_at=null/status=active仮想行）。深夜跨ぎは実datetime（0-9時台=翌暦日）でstart<end常時成立。`shift_start_at`カラムも追加し schedules.php 同期を start/end 両方に拡張（work=セット・休み/未定=NULL化・同値no-op。役割=updated_at を動かして bot に変更通知＋監査。GET値はschedules導出なのでカラムとGETは独立）。
- **P6**: ヒメ割の**ON/OFF廃止**（本日出勤あり=自動掲載・出勤終了=bot自動取消）。CTRLからチェックボックスと**終了時刻入力を削除**（「期限＝出勤表の終了と連動」読み取り専用表示のみ）。**P3任意4の逆方向同期（ヒメ割→出勤end_time）は撤去**＝終了の正は出勤表一本化。編集できるのは**分・円のみ**（null可=bot既定70分/11,000円）。himewari_enabled カラム/GETフィールドは互換で残置（bot非参照）。
- PUT: himewari_minutes/himewari_price 部分更新可。shift_* はPUTで受けてもGETに反映されない（出勤表を編集すること）。
- 本番E2E受け入れ 全PASS（pa行なしGET出現/翌日跨ぎ/play_at独立/出勤変更即反映/カラム同期no-op/分円のみ更新/休み→null/深夜順序）。
- ⚠️ クリア（status=cleared）の行は GET status=active から消える＝**即姫クリアするとその子のヒメ割/出勤帯もactive一覧から見えなくなる**。botが status=all で取得していれば問題なし（要bot側確認事項として報告済み）。

## 実装履歴（2026-07-13 後半）— 出勤表↔ヒメ割(shift_end_at)双方向連動（コミット 27004e23）

`references/CLAUDE-SHIFT-SYNC.md` 準拠。ヒメ割の意味＝出勤終了までに遊ぶ、なのに出勤表と独立で混乱していたのを連動化。
- **出勤→ヒメ割（必須1）**: `ctrl/schedules.php` $saveOne 内で、**本日営業日（5時区切り）のwork＋終了時刻あり**の保存時に同一キャストの `play_availability.shift_end_at` を同時刻に更新（end 0〜9時台=翌暦日）。himewari ON/OFF問わず揃える（次回ONの初期値に効く）。**既存行がある場合のみUPDATE**（即姫もヒメ割も未設定の子に空行を作らない）。**同値no-op**（updated_at据え置き＝botの無駄な再反映防止）。updated_by=`shift:{username}`（監査で出勤連動と分かる）。
- **ヒメ割→出勤（任意4・逆方向）**: `ctrl/play-availability.php` action=himewari で期限保存時、本日営業日のwork行の `end_time` も更新（**行が存在する時のみ・勝手に出勤行を作らない・同値no-op**）。flashに「出勤表の終了時刻も HH:MM に更新しました」を付加。
- **即姫は独立（必須3）**: play_at の保存(set/now/API PUT)は shift_end_at/出勤に一切触れない。逆も然り（受け入れT1/T2で実証）。
- 必須2（ヒメ割ON初期値=本日出勤終了）は既存実装のまま。ヘルプに「即姫=遊べる開始時刻・独立／ヒメ割=期限(出勤終了)・出勤表と連動」を明記。
- **本番DBレベル受け入れテスト11項目 ALL PASS**（空きキャスト自動選択→実データ保護ABORTガード→テスト後掃除。T1同期/T1b no-op/T2独立/T4逆方向/T4b no-op/T5空行不生成/cleanup）。
- 媒体POST・吉祥寺media-poster-cronは無変更。

## 実装履歴（2026-07-13）— play-availability に ヒメ割(P1)＋媒体ID fuzoku/deli(P2) 追加（コミット 3585be26）

`/Users/biyobu/test/official-media-update/references/CLAUDE-FULL-BRIEF.md`（Grok作成の全体指示書）のオフィシャル側追加依頼。**役割分担**: Claude=オフィシャル(admi2888.com)のUI/DB/API、Grok=媒体bot(official-media-update, 立川各媒体へPOST)、人間=設定。**吉祥寺 media-poster-cron は触らない・統合しない**（別系統：admi公開scrape→吉祥寺情報局）。媒体への直接POSTはしない。

- **DB**（両サイト共有・ALTER済み、migration_play_availability.sql更新）:
  - `play_availability`: `shift_end_at`(ヒメ割期限=出勤終了) / `himewari_enabled` / `himewari_minutes`(NULL=bot既定70) / `himewari_price`(NULL=既定11000) 追加。**`play_at` を NULL 可に変更**（即姫とヒメ割は同一行の独立ファセット＝どちらか片方だけもOK）。
  - `girl_media_ids`: `fuzoku_girl_no`(風じゃ) / `deli_girl_no`(デリじゃ) 追加（媒体別番号・girl_noは媒体で違う）。
- **API** `api/play-availability.php`:
  - GET items に `shift_end_at`(ISO)/`himewari_enabled`(bool)/`himewari_minutes`/`himewari_price`、`media_ids` に `fuzoku`/`deli` 追加。
  - PUT を**部分更新**に（`array_key_exists` で body 指定フィールドのみ変更・未指定は既存維持。既存行を読んでマージ→upsert）。**play_at必須の400を撤廃**（`{"himewari_enabled":true,"shift_end_at":...}` だけのPUTで play_at 維持）。5分丸めは play_at/shift_end_at 両方に適用。
- **CTRL** `/ctrl/play-availability.php`: 「ヒメ割」列（`details`：出す✓＋終了時刻ピッカー[本日出勤終了を初期値]＋分＋円、ON時は赤字サマリ）／媒体ID欄に風じゃ・デリじゃ入力追加／ヘルプに「情報局ヒメ割=1分」「風じゃ/デリじゃ=45分」追記。
- **回帰死守**: play_at 5分丸め・クリア→active一覧消失・updated_at自動更新・既存media_ids(fujoho/ekichika/heaven)、全て維持。本番E2E全PASS（17:00丸め／ヒメ割のみPUTでplay_at維持／part更新でshift_end保持／クリア0件／fuzoku・deliキー、テスト行掃除済み）。CTRL 302・API 200。
- **未実装（意図的・briefのP3）**: Webhook/dispatchキュー（初版はbotポーリングで足りる）、ヘブン「ここぞ」ワンクリック（将来）。
- ⚠️ 即姫とヒメ割で `play_at` NULL 可になった＝bot側は play_at=null のキャスト（ヒメ割のみ）を即姫ジョブでスキップする想定。

## 実装履歴（2026-07-12）— CTRL「最速で遊べる時間」（即ヒメ正データ）＋bot連携API（コミット 977b7015）

媒体更新ツール構想（[[project_admi_media_sync_tool]]）の第一歩。`/Users/biyobu/test/official-media-update/references/official-ui-brief-for-claude.md` 準拠。**この画面が正（Single Source of Truth）**、情報局/駅ちか/ヘブンへの実反映は別bot（Grok側）が本APIをポーリングして行う（オフィシャルから媒体へ直接POSTしない）。

- **DB**（sql/migration_play_availability.sql、サーバーで直接CREATE済み・両サイト共有DB）:
  - `play_availability`: shop_id×girl_id UNIQUE（1キャスト有効1件・upsert）、play_at(JST DATETIME)、status(active/cleared)、list_flag、note、updated_by、**updated_at自動更新=bot変更検知**。cleared更新でもupdated_atが変わる＝取消もbotが検知できる。
  - `girl_media_ids`: shop_id×girl_id で fujoho_girl_id / ekichika_girl_id / heaven_member_id（媒体アカウントは店舗別＝立川と吉祥寺で別ID）。
- **CTRL** `/ctrl/play-availability.php`（ナビ=媒体連携グループ・⏰）: 一覧（本日出勤を上に+出勤バッジ、営業日5時区切り）/ プレビュー（play_at<=now→「🔥今すぐ遊べる(即姫)」・未来→「HH:mmから遊べる」・未設定/cleared→「—」）/ 時刻設定(5分刻みselect・schedules同型) / 今すぐ / クリア / 媒体IDインライン編集(details折りたたみ) / 更新日時+操作者表示 / 媒体反映ルールのヘルプ表。
  - **確定仕様**: 「今すぐ」=現在時刻を**5分切り下げ**（即 play_at<=now で即姫成立）。時刻指定で選択時刻が現在より過去→**翌日扱い**（過去にしたい時は「今すぐ」を使う）。
- **API** `api/play-availability.php`（bot連携・両サイト配信）:
  - 認証: `X-Api-Key` ヘッダー（or ?key=）＝ db-config.php の `PLAY_API_KEY`（サーバーで新規生成済み・両サイト同値）。未設定503/不一致401。
  - `GET ?shop_id=1[&status=active|cleared|all][&updated_since=ISO8601][&cast_id=N]` → `{items:[{cast_id,name,play_at(ISO+09:00),status,list_flag,note,updated_at,updated_by,media_ids:{fujoho,ekichika,heaven}}],server_time}`
  - `PUT/POST ?shop_id=1&cast_id=N` + JSON `{play_at?,status?,list_flag?,note?,updated_by?}` → upsert（play_atは5分切り下げ丸め、girl_shopsで越境防止404、`{"status":"cleared"}`のみでクリア可）。
  - パスパラメータ形式（/{cast_id}）はシンレンApacheの都合でクエリ形式に統一。dispatchキュー(4.5)は未実装（初版はupdated_atポーリングで足りる）。
- **本番E2E実測**: 認証なし/誤キー401・正キー200・PUT 17:03→**17:00丸め**・GET(name/media_ids付き)・updated_since差分・単体取得・クリア→active一覧から消えupdated_at更新・存在しないcast404、全PASS（テスト行は掃除済み）。
- ⚠️ **教訓**: `api/db.php` は db-config.php を **DB::conn()の中で遅延require** する。DB接続前に db-config の定数（PLAY_API_KEY等）を使うAPIは、**冒頭で `require_once __DIR__.'/db-config.php'` を明示**しないと常に未定義（初回503の原因だった）。translate.php はDB照会後にキーを見るため偶然動いていた。
- 配信: PHPのみのため直接rsync（api→両サイト、ctrl→admi2888のみ）。正規デプロイ（deploy.sh/deploy-prod）にも同梱される。
