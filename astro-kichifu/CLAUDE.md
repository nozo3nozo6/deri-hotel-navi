...
- **残（ユーザーがCF/GSCで）**: GSCで sitemap-index.xml 送信・主要ページURL検査/インデックス要求・ドメインプロパティ化、Bingインポート、CF（robots/sitemapのキャッシュ対策＝パージ or Cache Rule、SSL Full/Always HTTPS/www↔apex 301統一）。詳細 [[feedback_astro_sitemap_setup]]。
