...
#### GSC全カテゴリ分析（2026-06-18調査）
- **対象総数**: 約3,650件（インデックス3,136件 + 未登録514件）
- **登録済み 3,136件**: 正常。ブランドKWで1位、長尾KWでも3位実績あり
- **意図的な未登録 141件（正常・変更不要）**:
  - noindex 12件: admin/shop-admin等の管理画面（意図通り）
  - robots.txt除外 5件: /api/ 等（意図通り）
  - 301リダイレクト 61件: 旧URL /men/→/deli/ 等（意図通り）
  - canonical統合 63件: /jofu/同性モード等の旧サブドメイン代替（意図通り）
- **クロール済み-未登録 372件（正常・コード修正不要）**:
  - 大半は口コミ0件のホテル詳細ページ（コンテンツ薄→Googleが保留）
  - 一部はsame-m/same-f/esteの都道府県・市区町村ページ（ニッチKW=低優先）
  - 対策は口コミ増加のみ（コード変更では解決しない）
- **アクション要 4件 → 実質ゼロ**:
  - `5xx (サーバーエラー)` 1件: `/deli/hotel/8287` → **2026-06-18 GSC URL検査で「インデックス登録済み」を確認、解決済み**（一時的なDB/サーバー負荷が原因、現在200で正常、コード修正不要）
  - `404 (未検出)` 1件: 削除済みURLの残存、再クロール待ち（自然消滅待ち・放置可）
...
### 2026年6月25日 — 店舗ページ メインエリアUI調整・掲載店バッジ常時表示化

#### 店舗専用URL「📍 メインエリア」カードのレイアウト最終調整（api-service.js `renderShopServiceAreaTags()`）
- **★・ゴールド強調の撤去**: メイン(立川市)だけ★+ゴールド装飾だと「ここ以外はおすすめでない」と誤解されるため、全カードを均一デザインに統一（is_primary 視覚差を廃止）
- **フォントサイズのバランス調整**: 「文字数が多いラベルほど緩やかに縮小」方式に決定
  - `effLen()`: ラベルの見た目長を概算（全角=1.0, ・=0.6, 半角空白=0.3）
  - `fontSizeFor()`: 段階フォント（≤4=0.82rem / ≤6=0.74rem / ≤8=0.68rem / ≤10=0.62rem / 他=0.56rem）— 可読性優先で緩やか
  - 例: 立川市・相模=大きめ、新宿・中野・荻窪・四谷=小さめ
- **長いラベルは改行で対応**（1行強制のフォント過縮小・文字欠けを回避）
- **箱の高さを実測で統一**: `equalizeShopAreaCards()` が描画後(rAF×2)に全カードの自然高さを測り最も高いカードへ min-height を揃える。ビューポート幅で行数が変わるためリサイズ時も再計算（リスナーは window._shopAreaEqualizeBound で1回だけ登録）
- **矢印「›」対策**: カードに padding-right:20px を確保し改行時も文字が矢印に被らない
- 試行錯誤の経緯: 固定rem→文字欠け→実測1行フィット→フォントばらつき→均一フォント+改行→箱高さばらつき→「文字数多→緩やか縮小+箱高さ統一」で確定
- セクション順序: ヘッダー → H1ヒーロー(#genre-hero) → 📍メインエリア → 地域選択（挿入アンカーは `#genre-hero || header`）

#### YobuChat 掲載店バッジ — 常時表示化・店舗側トグル廃止
- **方針**: 無料YobuChat利用の対価として「🏨 YobuHo掲載店」リンク（チャットフッターの被リンクSEO導線）を常時表示・必須化。店舗側のON/OFF選択制を廃止
- **変更**:
  - `api/chat-api.php` `handleBadgeInfo()`: `show_badge` ゲートを撤去し、shop存在のみ確認して常時 `enabled:true`（show_badgeカラムは残存・未参照）
  - `api/chat-api.php`: 不要になった `admin-toggle-badge` ルート/`handleAdminToggleBadge()` を削除
  - `shop-admin.js`: バッジ表示トグルのハンドラ削除
  - `shop-admin.html`: トグルUI → 一旦案内文に置換後、**案内文も削除**（店舗への説明・選択肢を一切出さない）
- バッジリンク自体（チャットフッター）は静的`a[data-ychat-badge]`同梱 + chat-embed.js自動注入で従来通り。技術的に個別無効化が必要な場合のみ iframe に `data-ychat-badge-off="1"`（shop-admin非露出）
...
- ②iframe: 静的 `<iframe data-ychat-slug>` + `<script src="yobuho.com/chat-embed.js">` 1行（外部ブリッジ）
- ③link: 別タブリンク（iframe 不使用）
- ④floating: 別タブ浮動リンク（iframe 不使用）
- ⑤CMS用インライン: ②と同じ `<iframe data-ychat-slug>` + chat-embed.js。script 禁止 CMS 用に固定高さ legacy 版も別提供

**外部ブリッジ chat-embed.js（postMessage 受信側、2026-04-24 導入）:**
- 顧客HPに貼る snippet から inline script を完全排除 → CMS の description auto-extraction（例: 秘密基地系 Go-Kichi CMS）が script 内容を meta に埋める事故を防止
- `document.querySelectorAll('iframe[data-ychat-slug]')` で全 iframe を自動ワイヤリング、MutationObserver で遅延挿入にも対応
