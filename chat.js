...
// 店舗 slug が無く 404 になる. PHP エンドポイント /api/chat-manifest.php?slug=...&inbox=... を経由して
// slug 付き start_url を返す動的 manifest を取得. iOS Safari は Blob URL の manifest を確実にサポート
// しないため real HTTP URL 経由とする.
(function injectDynamicManifest() {
    try {
        const slug = SLUG;
        if (!slug) return; // slug 無しなら静的 manifest のまま
        let manifestUrl = '/api/chat-manifest.php?slug=' + encodeURIComponent(slug);
        if (CAST_INBOX_TOKEN) {
            manifestUrl += '&inbox=' + encodeURIComponent(CAST_INBOX_TOKEN);
        }
