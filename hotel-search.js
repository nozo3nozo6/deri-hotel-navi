...
function toggleAccordionForm(id) {
    ...
    const arrowL = document.getElementById(arrowId + '-l');
    const rot = isOpen ? '' : 'rotate(180deg)';
    if (arrow) arrow.style.transform = rot;
    if (arrowL) arrowL.style.transform = rot;
}
...
function renderLovehoDetail(hotel, reports) {
    ...
    function buildLhReviewCard(r) {
        ...
        const shopSoloEntryHTML = r.solo_entry && isShopPost && _siActive ? (r.solo_entry==='yes' ? `<span class="round-badge round-badge--solo-can">🚪 一人で先に？</span>` : r.solo_entry==='together' ? `<span class="round-badge round-badge--solo-can">🚪 一緒に入室</span>` : r.solo_entry==='no' ? `<span class="round-badge round-badge--solo-ng">🚪 一人で先に？ 不可</span>` : '') : '';
