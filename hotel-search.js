...
async function fetchAndShowHotelsByCity(filterObj, city) {
    ...
    try {
        ...
        if (filterObj.prefecture) apiParams.pref = filterObj.prefecture;
        // 市区町村ページは経路に関係なく、その市の全ホテルを表示（major_areaでフィルタしない）
        ...
        sortHotelsByReviews(hotels);
        renderHotelCards(hotels);
        // 市区町村の合計件数はmajor_area横断で取得
        const cityTotal = getCityTotalFromAreaData(pref, null, city);
...
async function showLovehoTabs(pref, city, hotelCount, hotels, totalHotelCount) {
    ...
    const cacheKey = pref + '|||' + city + '|||' + (majorArea || '');
    ...
    // area-data.jsonからラブホ件数を取得（市区町村は全major_area合計）
    let lovehoCount = 0;
    let lovehoFound = false;
    if (_areaData) {
        for (const [key, areaInfo] of Object.entries(_areaData.area || {})) {
    ...
    const lovehoTab = lovehoCount
        ? `<button class="hotel-tab detail-tab detail-tab--inactive" data-tab="loveho" onclick="switchTab('loveho')">🏩 ラブホ <span class="tab-badge" id="loveho-count">${lovehoCount}</span></button>`
        : '';
...
async function loadLovehoForCurrentCity() {
    ...
    // 市区町村ページは全major_areaのラブホを表示（major_areaでフィルタしない）
...
function getCityTotalFromAreaData(pref, majorArea, city) {
    ...
    // majorArea未指定時: 全エリアの合計を返す（市区町村ページ用）
    let totalHotel = 0, totalLoveho = 0, found = false;
    for (const [key, areaInfo] of Object.entries(_areaData.area || {})) {
        if (!key.startsWith(pref + '\t')) continue;
        const match = (areaInfo.ct || []).find(c => c[0] === city);
        if (match) { totalHotel += match[1]; totalLoveho += match[2]; found = true; }
    }
    return found ? { hotel: totalHotel, loveho: totalLoveho } : null;
}
...
async function executeKeywordSearch() {
    ...
    try {
        ...
        const lovehoTab = lovehoResults.length
            ? `<button class="hotel-tab detail-tab detail-tab--inactive" data-tab="loveho" onclick="switchKeywordTab('loveho')">🏩 ラブホ <span class="tab-badge">${lovehoResults.length}</span></button>`
            : '';
        tabsDiv.innerHTML = `
...
function renderDetailShopCards(shops, cityName) {
    ...
    const cards = shops.map((s, i) => {
        ...
        const prHtml = prText ? `<div style="font-size:12px;color:#444;margin-top:3px;padding:4px 8px;border-radius:4px;background:color-mix(in srgb, var(--accent,#b5627a) 6%, transparent);overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;line-height:1.3;">${esc(prText)}</div>` : '';
        const rank = i + 1;
...
function renderSubAdCards(ads, label) {
    ...
    const cards = ads.map((ad, i) => {
        ...
        const detailHtml = detailParts.length ? `<div style="display:flex;align-items:center;gap:6px;margin-top:2px;flex-wrap:wrap;">${detailParts.join('')}</div>` : '';
